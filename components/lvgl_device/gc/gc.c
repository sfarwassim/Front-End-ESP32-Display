 ...
 *
 * Creates a new allocation object using the system `pvPortMallc`.
 *
...
{
    Allocation* a = (Allocation*) pvPortMalloc(sizeof(Allocation));
    a->ptr = ptr;
    a->size = size;
    a->tag = GC_TAG_NONE;
    a->dtor = dtor;
...
static AllocationMap* gc_allocation_map_new(size_t min_capacity,
        ...
        double upsize_factor)
{
    AllocationMap* am = (AllocationMap*) pvPortMalloc(sizeof(AllocationMap));
...
{
    if (!count) return pvPortMalloc(size);
...
{
    /* Allocation logic that generalizes over pvPortMalloc/calloc. */
